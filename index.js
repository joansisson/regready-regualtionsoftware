...
var init_db = __esm({
  "server/db.ts"() {
    "use strict";
    init_schema();
    sqlitePath = process.env.REGREADY_DB_PATH ? process.env.REGREADY_DB_PATH : path.join(process.cwd(), "local.db");
  ...
  "server/database-storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async createVerifiedLink(link) {
        const [newLink] = await db.insert(verifiedLinks).values(link).returning();
        return newLink;
      }
...
async function registerRoutes(app2) {
  ...
  app2.get(
    ...
    requireAuth,
    asyncHandler(async (req, res) => {
      const policy = await storage.getPolicy(parseInt(req.params.id), req.user.organizationId);
  ...
  app2.put(
    ...
    requireAuth,
    asyncHandler(async (req, res) => {
      const updatedPolicy = await storage.updatePolicy(parseInt(req.params.id), req.body, req.user.organizationId);
  ...
  app2.post(
    ...
    requireAuth,
    asyncHandler(async (req, res) => {
      const report = await storage.getAuditReport(parseInt(req.params.id), req.user.organizationId);
...
app.use(
  helmet({
    ...
    // CSP header is set by our middleware
...
function scheduleGeneratedPdfCleanup() {
  ...
  const dir = path6.join(baseDir, "generated-pdfs");
  const cleanupOnce = () => {
    try {
      ...
      for (const name of entries) {
        ...
        try {
          ...
          if (stat.isFile() && stat.mtimeMs < cutoffMs) fs3.unlinkSync(filepath);
        } catch {
        }
      }
    } catch {
...
(async () => {
  try {
    ...
    const staticDir = path6.resolve(baseDir, "dist", "public");
    const indexHtmlPath = path6.join(staticDir, "index.html");
    const canServeStatic = fs3.existsSync(indexHtmlPath);
    if (canServeStatic) {
      ...
      console.warn(
        `[${APP_NAME}] Static client not found. baseDir=${baseDir} staticDir=${staticDir} indexHtml=${indexHtmlPath}. Falling back to Vite middleware.`
      );
    ...
    server.listen(port, "0.0.0.0", () => {
      log2(`\u{1F680} Local Pro Engine active at http://localhost:${port}`);
    });
  } catch (error) {
