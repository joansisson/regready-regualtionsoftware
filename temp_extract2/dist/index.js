...
(async () => {
  try {
    ...
    try {
      fs3.readFileSync(indexHtmlPath);
      canServeStatic = true;
    } catch {
    ...
    if (canServeStatic) {
      ...
      console.warn(
        `[${APP_NAME}] Static client not found (read probe failed). baseDir=${baseDir} staticDir=${staticDir} indexHtml=${indexHtmlPath}. Falling back to Vite middleware.`
      );
