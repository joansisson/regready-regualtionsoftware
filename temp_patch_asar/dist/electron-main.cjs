...
async function createWindow() {
  ...
  if (startServerInMain) {
    ...
    process.env.REGREADY_BASE_DIR = app.isPackaged ? path.join(process.resourcesPath, "app.asar") : app.getAppPath();
    appendPackagedDebugLine(`REGREADY_BASE_DIR=${process.env.REGREADY_BASE_DIR}`);
    process.env.REGREADY_DB_PATH = app.isPackaged ? path.join(process.resourcesPath, "app.asar.unpacked", "local.db") : path.join(process.cwd(), "local.db");
    ...
    appendPackagedDebugLine(`computed stableElectronExecPath=${stableElectronExecPath}`);
    ...
    serverProcess.stdout?.on(
      "data",
      (d) => console.log(`[server] ${d.toString("utf8").trimEnd()}`)
    );
    serverProcess.stderr?.on(
      "data",
      (d) => console.error(`[server] ${d.toString("utf8").trimEnd()}`)
    );
  }
